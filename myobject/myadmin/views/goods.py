...
def index(request,pIndex):
    ...
    if kw:
        # 查询商品名中只要含有关键字的都可以
        list = Goods.objects.filter(goods__icontains=kw)
...
def insert(request):
    try:
        ...
        # 以时间戳命名一个新图片名称
        filename= str(time.time())+"__"+myfile.name.split('.').pop()
        print(filename)
...
#    content= UEditorField('', width=600, height=300, toolbars="mini", imagePath="images/", filePath="files/", upload_settings={"imageMaxSize":1204000}, settings={})  
# 打开商品类别信息编辑表单
